...
data class InboundTransferContext(
    val request: TransferSessionRequest,
    val authenticatedPeerId: String,
)

fun interface IncomingApprovalGateway {
    suspend fun requestApproval(peer: PairedPeer, request: TransferSessionRequest): Boolean
}

fun interface PeerApprovalPolicyResolver {
    suspend fun policy(peer: PairedPeer): PeerApprovalPolicy
}

fun interface InboundTransferReceiver {
    suspend fun receive(
        peer: PairedPeer,
        request: TransferSessionRequest,
        events: TransferEventSink,
...
class InboundTransferSessionCoordinator(
    private val peerDirectory: PeerDirectory,
    private val approvalPolicyResolver: PeerApprovalPolicyResolver = PeerApprovalPolicyResolver {
        PeerApprovalPolicy.REQUIRE_APPROVAL
    },
    private val approvalGateway: IncomingApprovalGateway,
    private val receiver: InboundTransferReceiver,
    private val deadlineScheduler: TransferDeadlineScheduler,
    private val cancellation: TransferCancellationChecking,
    private val events: TransferEventSink,
    private val timeouts: TransferTimeoutPolicy = TransferTimeoutPolicy(),
) {
    suspend fun execute(context: InboundTransferContext): RoleTransferOutcome {
        val request = context.request
        return try {
            val peer = peerDirectory.pairedPeer(context.authenticatedPeerId)
                ?: return terminal(TransferTerminalResult.REJECTED, TransferActivityPhase.REJECTED, request)
            events.emit(event(TransferActivityPhase.AWAITING_APPROVAL, request))
            val approved = when (approvalPolicyResolver.policy(peer)) {
                PeerApprovalPolicy.AUTO_ACCEPT -> true
                PeerApprovalPolicy.REQUIRE_APPROVAL -> deadlineScheduler.run(timeouts.approvalMillis) {
                    approvalGateway.requestApproval(peer, request)
                }
            }
            if (!approved) return terminal(TransferTerminalResult.REJECTED, TransferActivityPhase.REJECTED, request)
            if (cancellation.isCancelled()) {
                return terminal(TransferTerminalResult.CANCELLED, TransferActivityPhase.CANCELLED, request)
            }
            events.emit(event(TransferActivityPhase.TRANSFERRING, request))
            val artifact = receiver.receive(
                peer, request, events, cancellation, deadlineScheduler, timeouts.idleMillis,
            )
            events.emit(event(TransferActivityPhase.COMPLETED, request, verified = request.totalBytes()))
            RoleTransferOutcome(TransferTerminalResult.COMPLETED, listOf(artifact))
        } catch (error: TransferExecutionException) {
            events.emit(event(TransferActivityPhase.FAILED, request))
            RoleTransferOutcome(
                result = TransferTerminalResult.FAILED,
